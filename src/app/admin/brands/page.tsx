...
import { 
  Plus, Search, SlidersHorizontal, MoreVertical, ExternalLink,
  Pencil, Eye, EyeOff, Trash2, ChevronLeft, ChevronRight,
  ArrowUpDown, Download, CheckCircle, XCircle, Award
} from "lucide-react"
...
export default function BrandsPage() {
  ...
  const visibilityMutation = useMutation({
    mutationFn: async (brand: Brand) => {
      const res = await fetch('/api/brands', {
        ...
        body: JSON.stringify({ id: brand.id, visibility: !brand.visibility, name: brand.name, slug: brand.slug, website: brand.website })
      })
      const json = await res.json()
  ...
  const handleDelete = (id: string) => {
    if (confirm("Permanently purge this brand identity?")) {
      deleteMutation.mutate(id)
    }
  }

  const toggleVisibility = (brand: Brand) => {
    visibilityMutation.mutate(brand)
  }
  ...
  const columns = React.useMemo(() => [
    ...
    columnHelper.accessor('name', {
      ...
      cell: info => (
        <div className="flex flex-col">
          ...
          <span className="text-[10px] font-bold text-gray-400 opacity-60">ID: ATL-{info.row.original.id}</span>
        </div>
      )
    }),
    columnHelper.accessor('website', {
      ...
      cell: info => {
        ...
        if (!url) return <span className="text-xs font-bold text-gray-300 italic">—</span>
        return (
          <a href={url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline text-sm font-bold flex items-center gap-1.5 transition-all">
          ...
          <div className="text-right">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48 bg-white dark:bg-gray-950 p-1.5 rounded-xl shadow-2xl border-none z-50">
                 <DropdownMenuGroup>
                    <DropdownMenuItem onClick={() => window.open(brand.website, '_blank')} className="flex items-center gap-2 p-2.5 rounded-lg font-bold text-xs uppercase tracking-widest cursor-pointer hover:bg-blue-50 hover:text-blue-600">
                       <ExternalLink className="size-3.5" /> Visit website
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => toggleVisibility(brand)} className="flex items-center gap-2 p-2.5 rounded-lg font-bold text-xs uppercase tracking-widest cursor-pointer hover:bg-gray-50">
                       {brand.visibility ? <EyeOff className="size-3.5" /> : <Eye className="size-3.5" />} Toggle visibility
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => window.location.href = `/admin/brands/${brand.id}/edit`} className="flex items-center gap-2 p-2.5 rounded-lg font-bold text-xs uppercase tracking-widest cursor-pointer hover:bg-gray-50">
                       <Pencil className="size-3.5" /> Edit Identity
                    </DropdownMenuItem>
                    <DropdownMenuSeparator className="my-1.5 opacity-10" />
                    <DropdownMenuItem onClick={() => handleDelete(brand.id)} className="flex items-center gap-2 p-2.5 rounded-lg font-bold text-xs uppercase tracking-widest text-rose-600 cursor-pointer hover:bg-rose-50">
                       <Trash2 className="size-3.5" strokeWidth={3} /> Purge Record
                    </DropdownMenuItem>
                 </DropdownMenuGroup>
              </DropdownMenuContent>
            </DropdownMenu>
    ...
    <DashboardLayout>
      <div className="flex flex-col gap-8 font-sans">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-2">
          <div className="space-y-1">
             <nav className="flex items-center gap-2 text-[10px] uppercase font-bold tracking-[0.2em] text-gray-400">
               ...
               <span className="text-gray-900 dark:text-white">Registry</span>
             </nav>
          ...
          <div className="flex flex-wrap items-center gap-3">
             <Button variant="outline" className="h-11 px-6 font-bold text-[10px] uppercase tracking-widest rounded-xl hover:bg-gray-50 border-gray-200 shadow-sm">
                <Download className="size-4 mr-2 opacity-50" /> Export Index
             </Button>
             <Link href="/admin/brands/create">
               <Button className="h-11 px-6 bg-primary hover:bg-primary/90 text-primary-foreground font-black text-[10px] uppercase tracking-widest rounded-xl shadow-lg shadow-primary/20 active:scale-95 transition-all">
                  <Plus className="size-4 mr-2" /> New Identity
               </Button>
             </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {[
            { label: "Total Assets", val: items.length, icon: Award, color: "text-blue-600" },
            { label: "Public Status", val: items.filter(b => b.visibility).length, icon: CheckCircle, color: "text-emerald-600" },
            { label: "Restricted", val: items.filter(b => !b.visibility).length, icon: XCircle, color: "text-rose-600" },
            { label: "Last Sync", val: new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), icon: Search, color: "text-amber-600" }
          ].map((s, i) => (
        ...
        <div className="bg-card border border-border/50 p-4 md:p-8 rounded-2xl md:rounded-[32px] shadow-sm space-y-6 md:space-y-8">
           ...
           <div className="rounded-2xl border border-gray-50 dark:border-gray-800 overflow-x-auto overflow-y-hidden">
              <table className="w-full text-left">
                        ...
                        {headerGroup.headers.map(header => (
                          <th 
                            ...
                            className={cn(
                              ...
                              header.id === 'website' && "hidden lg:table-cell",
                 ...
                 <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                    {isLoading ? (
                       <tr><td colSpan={columns.length} className="py-20 text-center text-[10px] opacity-10 font-black tracking-[0.3em] uppercase">Synchronizing...</td></tr>
                    ) : table.getRowModel().rows.length === 0 ? (
                       <tr><td colSpan={columns.length} className="py-20 text-center text-[10px] opacity-10 font-black tracking-[0.3em] uppercase">No records found</td></tr>
                    ) : (
                       table.getRowModel().rows.map(row => (
                         <tr key={row.id} className="group hover:bg-secondary/5 transition-colors cursor-pointer" onClick={() => window.location.href = `/admin/brands/${row.original.id}/edit`}>
                           {row.getVisibleCells().map(cell => (
                             <td 
                               key={cell.id} 
                               className={cn(
                                 "px-3 md:px-8 h-auto py-3 md:py-6 first:rounded-l-2xl last:rounded-r-2xl align-middle",
                                 cell.column.id === 'website' && "hidden lg:table-cell",
           ...
           <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">
                Found {table.getFilteredRowModel().rows.length} global entities
              </span>
