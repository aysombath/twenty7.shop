  ...
  const visibilityMutation = useMutation({
    mutationFn: async (cat: Category) => {
      const res = await fetch('/api/categories', {
        ...
        body: JSON.stringify({ id: cat.id, name: cat.name, slug: cat.slug, description: '', visibility: !cat.visibility })
      })
      const json = await res.json()
  ...
  const columns = React.useMemo(() => [
    ...
    columnHelper.accessor('name', {
      ...
      cell: info => (
        <div className="flex flex-col">
          ...
          <span className="text-[10px] font-bold text-gray-400 opacity-60">/{info.row.original.slug}</span>
        </div>
      )
    }),
    columnHelper.accessor('parent_name', {
      ...
      cell: info => info.getValue() ? (
        <span className="text-xs font-bold text-gray-600">{info.getValue()}</span>
      ) : (
        <span className="text-[10px] font-bold text-gray-300 italic uppercase tracking-widest">—</span>
      )
    }),
          ...
          <div className="text-right">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48 bg-white dark:bg-gray-950 p-1.5 rounded-xl shadow-2xl border-none z-50">
                <DropdownMenuGroup>
                  <DropdownMenuItem onClick={() => toggleVisibility(cat)} className="flex items-center gap-2 p-2.5 rounded-lg font-bold text-xs uppercase tracking-widest cursor-pointer hover:bg-gray-50">
                    {cat.visibility ? <EyeOff className="size-3.5" /> : <Eye className="size-3.5" />} Toggle visibility
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => window.location.href = `/admin/categories/${cat.id}/edit`} className="flex items-center gap-2 p-2.5 rounded-lg font-bold text-xs uppercase tracking-widest cursor-pointer hover:bg-gray-50">
                    <Pencil className="size-3.5" /> Edit
                  </DropdownMenuItem>
                  <DropdownMenuSeparator className="my-1.5 opacity-10" />
                  <DropdownMenuItem onClick={() => handleDelete(cat.id)} className="flex items-center gap-2 p-2.5 rounded-lg font-bold text-xs uppercase tracking-widest text-rose-600 cursor-pointer hover:bg-rose-50">
    ...
    <DashboardLayout>
      <div className="flex flex-col gap-8 font-sans">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-2">
          <div className="space-y-1">
             <nav className="flex items-center gap-2 text-[10px] uppercase font-bold tracking-[0.2em] text-gray-400">
               <Link href="/admin/categories" className="hover:text-primary transition-colors">Taxonomy</Link>
          ...
          <div className="flex flex-wrap items-center gap-3">
             <Button variant="outline" className="h-11 px-6 font-bold text-[10px] uppercase tracking-widest rounded-xl hover:bg-gray-50 border-gray-200 shadow-sm">
                <Download className="size-4 mr-2 opacity-50" /> Export List
             </Button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {[
            { label: "Internal Items", val: categories.length, icon: Layers, color: "text-blue-600" },
            { label: "Public Tags", val: categories.filter(i => i.visibility).length, icon: CheckCircle, color: "text-emerald-600" },
            { label: "Orphans", val: categories.filter(i => !i.parent_id).length, icon: XCircle, color: "text-rose-600" },
            { label: "Active Modified", val: categories.filter(i => i.updated_at && (new Date().getTime() - new Date(i.updated_at).getTime() < 86400000)).length, icon: Pencil, color: "text-amber-600" }
          ].map((s, i) => (
        ...
        <div className="bg-card border border-border/50 p-4 md:p-8 rounded-2xl md:rounded-[32px] shadow-sm space-y-6 md:space-y-8">
           ...
           <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="relative group flex-1 max-w-full md:max-w-md">
                 ...
                 <Input 
                   placeholder="Search categorical registry..." 
                   value={globalFilter ?? ""}
           ...
           <div className="rounded-2xl border border-gray-50 dark:border-gray-800 overflow-x-auto overflow-y-hidden">
              <table className="w-full text-left">
                        ...
                        {headerGroup.headers.map(header => (
                          <th 
                            ...
                            className={cn(
                              ...
                              header.id === 'parent_name' && "hidden lg:table-cell",
                 ...
                 <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                    {isLoading ? (
                       <tr><td colSpan={columns.length} className="py-20 text-center text-[10px] opacity-10 font-black tracking-[0.3em] uppercase">Querying hierarchy...</td></tr>
                    ) : table.getRowModel().rows.length === 0 ? (
                       <tr><td colSpan={columns.length} className="py-20 text-center text-[10px] opacity-10 font-black tracking-[0.3em] uppercase">Registry empty</td></tr>
                    ) : (
                       table.getRowModel().rows.map(row => (
                         <tr key={row.id} className="group hover:bg-secondary/5 transition-colors cursor-pointer" onClick={() => window.location.href = `/admin/categories/${row.original.id}/edit`}>
                           {row.getVisibleCells().map(cell => (
                             <td 
                               key={cell.id} 
                               className={cn(
                                 "px-3 md:px-8 h-auto py-3 md:py-6 first:rounded-l-2xl last:rounded-r-2xl align-middle",
                                 cell.column.id === 'parent_name' && "hidden lg:table-cell",
           ...
           <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">
                Displaying {table.getFilteredRowModel().rows.length} taxonomic records
              </span>
